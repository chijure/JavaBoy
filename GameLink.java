...
class GameLink implements Runnable {
 ...
 /** The Game Link server is bound and running */
 ...
 public GameLink(Frame parent, String ip) {
  this.parent = parent;
  try {
 ...
 public void run() {
  ...
  while ((serverRunning) || (clientConnected)) {
...
//    System.out.println(v + " " + dmgcpu.instrCount);
     if (dmgcpu != null) {
      while ( /*(v + 600 > dmgcpu.instrCount) || /*((clock != -1) && (clock > dmgcpu.instrCount)) ||*/ (!dmgcpu.interruptsEnabled) || ((dmgcpu.ioHandler.registers[0x0F] & dmgcpu.INT_SER) != 0)) {
       try {
